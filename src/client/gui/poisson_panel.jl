function draw_poisson_panel(gui_data::GuiData, app_data::Model.AppData, sock::Comm.SocClient)
    if CImGui.TreeNode("Poisson Patterns")
        ...
        if returned
            Model.set_firing_rate!(app_data.model, gui_data.buffer)
        end
        ...
        if returned
            Model.set_poisson_pattern_min!(app_data.model, gui_data.buffer)
        end
        CImGui.SameLine(450)
        ...
        if returned
            Model.set_poisson_pattern_max!(app_data.model, gui_data.buffer)
        end
        ...
        if returned
            Model.set_poisson_pattern_spread!(app_data.model, gui_data.buffer)
        end
